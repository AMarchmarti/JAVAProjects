...
import org.lasencinas.CotxoxSpringBoot.domain.PoolConductores;
...
import org.lasencinas.CotxoxSpringBoot.service.CarreraService;
import org.lasencinas.CotxoxSpringBoot.service.ConductorService;
...
public class CotxoxApplicationTests {
	...
	public void test_save_carrera() {
		...
		assertEquals("1234567890123456", carreraService.recuperaCarrera(idCarrera).getTarjetaCredito());
	}
	...
	public void test_recuperar_conductor() {
		...
		Assert.assertNotNull(conductor);
		assertEquals("Samantha", conductor.getNombre());
	}
	...
	public void test_save_conductor() {
        ...
        for (Conductor conductora : poolConductores) {
            ...
            conductorService.crearConductor(conductora);
            index++;
        }

		assertEquals("Sabrina", conductorService.recuperarConductor("2222222222222222").getNombre());
		assertEquals("Cici", conductorService.recuperarConductor("3333333333333333").getNombre());
	...
	public void test_BooleanOcupado_adaptado_a_SQL() {
		...
		assertEquals("Samantha", conductora.getNombre());
		assertEquals(false, conductora.isOcupado());
		conductora.setOcupado(true);
		assertEquals(true, conductora.isOcupado());
	}
	...
	public void test_post_construct_servei_conductor() {
		...
		conductorService.guardarConductores();

		assertEquals("Sabrina", conductorService.recuperarConductor("2222222222222222").getNombre());
		assertEquals(false,conductorService.recuperarConductor("2222222222222222").isOcupado());
		assertEquals("Cici", conductorService.recuperarConductor("3333333333333333").getNombre());
		assertEquals(false,conductorService.recuperarConductor("3333333333333333").isOcupado());
	}
	...
	public void test_recuperar_conductor_libre_repositori() {
		...
		assertEquals("Samantha", conductoresLibres.get(0).getNombre());
		assertEquals(false, conductoresLibres.get(0).isOcupado());

		// introduïm més conductores a la BBDD

		conductorService.guardarConductores();
		conductoresLibres = conductorRepo.findByOcupado(0);
		assertEquals(3, conductoresLibres.size());
		assertEquals(false, conductoresLibres.get(1).isOcupado());
	}
	...
	public void test_asignar_conductor() {
		...
		assertEquals("Samantha", carreraService.recuperaCarrera(idCarrera).getConductor().getNombre());
	}
